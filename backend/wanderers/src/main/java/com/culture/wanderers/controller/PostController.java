...
public class PostController {
    ...
    public Post createPost(
        ...
        if ("FREE".equals(post.getType()) || post.getRegionTag() == null || post.getRegionTag().isBlank()) {
            post.setRegionTag(null);
        } else {
            post.setRegionTag(post.getRegionTag().trim());
        }
        post.setViewCount(0);
    ...
    public Post updatePost(
        ...
        post.setRegionTag(resolveRegionTag(post.getType(), detail.getRegionTag()));

        return postRepository.save(post);
    }

    private String resolveRegionTag(String type, String regionTag) {
        if ("FREE".equals(type) || regionTag == null || regionTag.isBlank()) {
            return null;
        }
        return regionTag.trim();
    }
