...
public class FestivalInsightService {
    ...
    public List<FestivalSummaryDto> summarize(List<Festival> festivals) {
        ...
        return deduplicateFestivals(festivals).stream()
    ...
    public List<FestivalSummaryDto> getPopularFestivals(int limit) {
        ...
        return deduplicateFestivals(festivalRepository.findAll()).stream()
    ...
    private FestivalSummaryDto toSummary(Festival festival, MetricsSnapshot metrics, boolean popular) {
        ...
        long saveCount = metrics.saveCountByFestivalId.getOrDefault(festivalId, 0L);
        ...
        long popularityScore = calculatePopularityScore(reviewCount, likeCount, saveCount, viewCount, partyCount);
    ...
    private MetricsSnapshot buildMetricsSnapshot() {
        ...
        Map<Long, Long> saveCountByFestivalId = new HashMap<>();
        ...
        for (UserSave save : userSaveRepository.findAll()) {
            if (!"festival".equalsIgnoreCase(save.getTargetType()) || save.getTargetId() == null) {
                continue;
            }

            saveCountByFestivalId.merge(save.getTargetId(), 1L, Long::sum);
        }
    ...
    private Set<Long> computePopularFestivalIds(MetricsSnapshot metrics) {
        ...
        return allFestivals.stream()
                .map(festival -> Map.entry(festival.getId(), calculatePopularityScore(
                        ...
                        metrics.saveCountByFestivalId.getOrDefault(festival.getId(), 0L),
    ...
    private long calculatePopularityScore(long reviewCount, long likeCount, long saveCount, long viewCount, long partyCount) {
        return (reviewCount * 5L) + (likeCount * 4L) + (saveCount * 4L) + (viewCount * 2L) + (partyCount * 3L);
    }

    private List<Festival> deduplicateFestivals(List<Festival> festivals) {
        ...
        for (Festival festival : festivals) {
            ...
            String key = dedupeKey(festival);
            unique.merge(key, festival, this::preferCanonicalFestival);
        }
        return new ArrayList<>(unique.values());
    }

    private Festival preferCanonicalFestival(Festival current, Festival candidate) {
    ...
    private String dedupeKey(Festival festival) {
        String title = normalizeDedupeText(festival.getTitle());
        String startDate = normalizeDedupeText(festival.getStartDate());
        String endDate = normalizeDedupeText(festival.getEndDate());
        ...
        return title + "|"
                + normalizeDedupeText(festival.getRegion()) + "|"
                + normalizeDedupeText(festival.getLocation());
    }
    ...
    private record MetricsSnapshot(
            ...
            Map<Long, Long> saveCountByFestivalId,
