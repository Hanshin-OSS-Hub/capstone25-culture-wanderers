...
const getFestivalDedupeKey = (festival) => {
  ...
  const endDate = normalizeDedupeText(festival.end_date);

  if (title && (startDate || endDate)) {
    return [title, startDate, endDate].join('|');
  }

  return [title, normalizeDedupeText(formatPlace(festival.region, festival.location))].join('|');
};

const preferFestivalCard = (current, candidate) => {
  const currentScore = Number(current.popularity_score || 0)
    + Number(current.view_count || 0)
    + Number(current.like_count || 0)
    + Number(current.review_count || 0)
    + Number(current.party_count || 0);
  const candidateScore = Number(candidate.popularity_score || 0)
    + Number(candidate.view_count || 0)
    + Number(candidate.like_count || 0)
    + Number(candidate.review_count || 0)
    + Number(candidate.party_count || 0);

  if (candidateScore !== currentScore) {
    return candidateScore > currentScore ? candidate : current;
  }

  return Number(candidate.id || Infinity) < Number(current.id || Infinity) ? candidate : current;
};

const dedupeFestivals = (festivalList) => {
  const unique = new Map();

  festivalList.forEach((festival) => {
    const key = getFestivalDedupeKey(festival);
    const current = unique.get(key);
    unique.set(key, current ? preferFestivalCard(current, festival) : festival);
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setFestivals(dedupeFestivals(normalized));
      } catch (error) {
