...
import { addFestivalSave, isFestivalSaved, removeFestivalSave } from '../utils/saveStorage';
import { addCalendarEvent } from '../utils/calendarStorage';
import { getFestivalBenefitMatches } from '../utils/festivalBenefitMatcher';
...
export default function Detail() {
    ...
    const fetchDetail = async () => {
      ...
      try {
        ...
        try {
          ...
          const savedOnServer = Array.isArray(saves)
            ? saves.some(
                (item) =>
                  String(item.targetType || '').toLowerCase() === 'festival' &&
                  Number(item.targetId) === Number(festivalData?.id)
          ...
          setSaved(isFestivalSaved(festivalData?.id));
        }
  ...
  useEffect(() => {
    const syncSaveState = () => {
      if (festival?.id != null) {
        setSaved(isFestivalSaved(festival.id));
      }
    };

    window.addEventListener('festival-saves-changed', syncSaveState);
    window.addEventListener('storage', syncSaveState);

    return () => {
      window.removeEventListener('festival-saves-changed', syncSaveState);
      window.removeEventListener('storage', syncSaveState);
    };
  }, [festival]);
  ...
  const getReviewTrustBadges = (review) => {
    const badges = Array.isArray(review.authorTrustBadges) ? [...review.authorTrustBadges] : [];
    const companionTrust = getCompanionTrust(review.authorEmail);
  ...
  const handleToggleSave = async () => {
    try {
      if (saved) {
        await authFetch(`/api/saves?targetType=festival&targetId=${festival.id}`, {
          method: 'DELETE',
        });
        removeFestivalSave(festival.id);
        setSaved(false);
        return;
      }

      await authFetch('/api/saves', {
        method: 'POST',
        body: JSON.stringify({
          targetType: 'festival',
          targetId: String(festival.id),
  ...
  const benefitMatches = getFestivalBenefitMatches(festival);
  ...
  const toggleVisitedFestival = async () => {
    try {
      ...
      if (!result.added) {
        navigate("/mypage/calendar?tab=past", {
          state: {
            notice: "이미 캘린더에 등록된 일정이에요. 지난 일정에서 방문 인증을 이어갈 수 있어요.",
          },
        });
        return;
      }
      ...
      navigate("/mypage/calendar?tab=past", {
        state: {
          notice: "오늘 날짜의 지난 일정으로 추가했어요. 인증을 완료하면 별점과 후기를 남길 수 있어요.",
        },
      });
    } catch (error) {
      console.error("내가 간 행사 처리 실패:", error);
      alert(error?.message || "로그인 후 이용할 수 있습니다.");
    }
  };
        ...
        <div className="detail-top">
          ...
          <div className="detail-summary">
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                ...
                <button
                  ...
                  aria-label={saved ? '저장 해제' : '저장 추가'}
                  title={saved ? '저장 해제' : '저장 추가'}
                  style={{
                    minWidth: '96px',
                    height: '44px',
                    borderRadius: '999px',
                    border: saved ? '1px solid #ff8fb1' : '1px solid #e5e7eb',
                    background: saved ? '#fff0f5' : '#ffffff',
                    color: saved ? '#ff4f86' : '#444',
                    cursor: 'pointer',
                    fontWeight: 700,
                    fontSize: '15px',
                    padding: '0 16px',
                    boxShadow: saved ? '0 2px 8px rgba(255, 143, 177, 0.18)' : 'none',
                  }}
                >
                  {saved ? '저장됨' : '저장'}
                </button>
            ...
            {benefitMatches.length > 0 ? (
              <div className="detail-benefit-box">
                <div className="detail-benefit-head">
                  <div>
                    <h2>이 행사에 적용되는 혜택 정보</h2>
                    <p>행사 장소나 문화공간이 혜택 대상에 정확히 포함될 때만 보여드려요.</p>
                  </div>
                  <button type="button" onClick={() => navigate("/benefits")}>
                    전체 혜택
                  </button>
                </div>
                <div className="detail-benefit-list">
                  {benefitMatches.map((benefit) => (
                    <button
                      type="button"
                      key={benefit.id}
                      className="detail-benefit-item"
                      onClick={() => {
                        if (benefit.url?.startsWith("/")) {
                    ...
                    >
                      {benefit.reason ? <em>{benefit.reason}</em> : null}
        ...
        <div className="detail-tab-panel">
          ...
          {activeTab === 'review' && (
            <section className="detail-section">
              ...
              <div className="review-trust-notice">
                방문 인증 리뷰 시스템: 상세 페이지에서 다녀왔어요를 누른 사용자만 리뷰를 작성할 수 있어요.
              </div>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  navigate(
                    `/mypage/reviews/new?targetType=festival&targetId=${encodeURIComponent(festival.id)}&targetTitle=${encodeURIComponent(festival.title)}`,
                    {
                      state: {
                        targetType: 'festival',
                    ...
                    {filteredReviews.map((review) => (
                      ...
                      >
                        ...
                        <div style={{ fontSize: '14px', color: '#666' }}>
                          ...
                          {getReviewTrustBadges(review).map((badge) => (
